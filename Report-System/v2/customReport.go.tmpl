...
{{$MESSAGE_LISTENER := CC-ID}} {{/* The CC-ID of your `.*` Regex trigger (the message listener) */}}
...
{{if and .CmdArgs (lt (len .CmdArgs) 2)}}
    {{if eq (index .CmdArgs 0) "dbSetup"}}
        {{if (in (split (index (split (exec "viewperms") "\n") 2) ", ") "ManageServer")}}
            {{dbSet .Guild.ID "reportSettings" (sdict "reportLog" $REPORT_LOG "reportDiscussion" $REPORT_DISCUSSION "messageListener" $MESSAGE_LISTENER)}}
