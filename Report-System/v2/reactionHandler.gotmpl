...
{{define "edit"}}
    ...
    {{if .notify}}
        {{$data := sdict "author" .author.ID "reason" (index $rEmbed.Fields 1).Value "target" (index $rEmbed.Fields 2).Value "info" .message}}
        {{dbSetExpire $moderator.ID "awaitMessage" $data 65}}
        {{$moderator.Mention}}: Please write a message down below. **You have 60 seconds.**
        {{scheduleUniqueCC .listener nil 75 (print $moderator.ID "_awaitMessage") $data}}
        {{deleteResponse}}
    {{end}}
{{end}}
...
{{if and ($embed := (index .ReactionMessage.Embeds 0)) (eq .Channel.ID $logs)}}
    ...
    {{if and $isMod (reFindAllSubmatches (toString $.User.ID) $embed.Footer.Text) $embed.Author $embed.Footer}}
        {{with $report}}
            ...
            {{else if eq $.Reaction.Emoji.Name "❗"}}
                {{$data = sdict "color" "message" "Report dismissed, warned for false report." "menu" "none" "emojis" (cslice "🏳️") "notify" false}}
            {{end}}
            {{$data.Set "rembed" $report}}
            {{$data.Set "log" $logs}}
            {{$data.Set "reportID" $.Message.ID}}
            {{$data.Set "author" $author}}
            {{$data.Set "listener" $msgl}}
